  ...
  if(docs){
    ...
    async.eachLimit(docs, 1, function(doc, next){
      ...
      if(completeDoc(doc)){
        ...
        //because command syns is not compulsory, it must contain the value of command where no other value exists
        if(!_.has(doc,'command_syns')){ doc.command_syns = [doc.command] ;  }
...
//function to remove commands per channel
function removeCommand(channel, command, callback){

  //command & channel must be lowercase
  var doc = {
  ...
  //get document for removal
  JIBU.pouch.get(_id).then(function(doc) {
    //remove document
    JIBU.pouch.remove(doc)
    .then(function(res) {
      callback(doc);
    });
...
function completeDoc(doc){
  ...
  var must_contain= ['channel','command','response'];
