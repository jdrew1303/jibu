...
function JIBU(options){
  ...
  this.options = _.extend(

    {
      ...
      expireIndexesIn: (1000 * 60 * 10), //expire keys every 10 minutes (in milliseconds)
      maxIndexCaches: 500
    },
  ...
  var cache_options = {
          max: this.options.maxIndexCaches,
          length: function (n, key) { return n * 2 + key.length },
          dispose: function (key, n) { n.close() },
          maxAge: this.options.expireIndexesIn
      };
...
/**
 * Function to Refresh Jibu Indexes
 * @param  {string} channel [description]
 */
function refreshIndex(channel){
  //lowercase channel
  channel = channel.toLowerCase();
  //delete key
  cache.del(channel);
