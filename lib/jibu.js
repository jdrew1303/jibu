...
function jibu(q,channel){
  ...
  // //if q is an object
  // var numericResults = formatNumericQuery(q);
...
function numericQuery(q){
  ...
  //all these on array...
  var res = qq.on(JIBU.index.index_numerals);
    ...
    if(completeDoc(doc)){
      ...
      //pick all numeric command_syns
       _.each(doc.command_syns, function(v){
          arr = _.map( v.match(/[0-9]+/ig), function(vv){ return parseFloat(vv); }) ;
